    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-neutral-800/80">
        ...
        <div className="flex items-center gap-3 self-start md:self-auto">
          <div className="px-4 py-2.5 rounded-xl bg-neutral-900 border border-neutral-800 text-neutral-300 font-mono text-xs font-semibold">
            <span className="text-[#d85d3a] font-bold text-base mr-1.5">{AI_IDEATION_VIDEOS.length}</span>
            Commercial Concepts
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-stretch sm:items-center">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full whitespace-nowrap transition-all ${
                selectedCategory === cat
                  ? 'bg-[#d85d3a] text-white font-bold shadow-md shadow-[#d85d3a]/25'
        ...
        <div className="relative min-w-[240px] max-w-sm">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 rounded-full bg-neutral-900 border border-neutral-800 text-xs text-white placeholder-neutral-500 focus:outline-none focus:border-[#d85d3a] transition-colors"
          />
        </div>
      </div>
      ...
      {filteredVideos.length === 0 ? (
        <div className="py-20 text-center space-y-3 bg-neutral-900/30 rounded-2xl border border-neutral-800/80">
          <p className="text-neutral-400 font-mono text-sm">No concept matching your search was found.</p>
          <button
            onClick={() => {
              setSelectedCategory('All');
              setSearchQuery('');
            }}
            className="px-4 py-2 rounded-full text-xs font-semibold text-white bg-[#d85d3a] hover:bg-[#c24e2d] transition-colors"
          >
            Reset Filters
          </button>
        </div>
            ...
            >
              ...
              <div className="relative aspect-video w-full overflow-hidden bg-black/60">
                ...
                <div className="absolute top-3 left-3">
                  <span className="px-2.5 py-1 rounded-full text-[10px] font-mono font-semibold uppercase tracking-wider bg-black/80 backdrop-blur-md text-[#d85d3a] border border-[#d85d3a]/40">
                    {video.category}
                  </span>
                </div>
              </div>
              ...
              <div className="p-5 flex-1 flex flex-col justify-between space-y-3 bg-[#090a0c]/80 border-t border-neutral-800/80">
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <h3 className="font-syne text-base font-bold text-white group-hover:text-[#d85d3a] transition-colors line-clamp-1">
                ...
                {/* Pipeline Tags */}
                <div className="pt-3 border-t border-neutral-800/80 flex flex-wrap items-center justify-between gap-2 font-mono text-[11px]">
                  <div className="flex flex-wrap gap-1">
                    {video.tools?.slice(0, 2).map((t, idx) => (
                      <span
                        key={idx}
                        className="px-2 py-0.5 rounded text-[10px] font-medium bg-neutral-800/90 text-neutral-300"
                      >
                        {t}
                      </span>
                    ))}
                  </div>
                  <span className="text-[#d85d3a] font-semibold group-hover:underline">
                    PLAY REEL ↗
                  </span>
                </div>
              </div>
