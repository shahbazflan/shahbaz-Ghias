...
interface AIConceptCardItemProps {
  video: VideoItem;
  index: number;
  onSelectVideo: (video: VideoItem) => void;
}

const AIConceptCardItem: React.FC<AIConceptCardItemProps> = ({ video, index, onSelectVideo }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const [containerSize, setContainerSize] = useState({ width: 380, height: 320 });
  ...
  const rotY = isHovered ? normX * 6 : 0;
  const rotX = isHovered ? -normY * 5 : 0;
  ...
  return (
    <div
      ...
      className="perspective-[1000px] select-none cursor-pointer"
    >
      <motion.div
        ...
        transition={{
          duration: isHovered ? undefined : 0.45,
          delay: isHovered ? undefined : index * 0.04,
          type: isHovered ? 'spring' : 'tween',
          damping: 24,
          stiffness: 220,
          mass: 0.5,
        }}
        className="group relative rounded-2xl overflow-hidden bg-neutral-900/70 border border-neutral-800 hover:border-[#d85d3a]/70 shadow-lg hover:shadow-2xl hover:shadow-[#d85d3a]/15 transition-[border-color,box-shadow] duration-300 flex flex-col justify-between will-change-transform transform-gpu"
      >
        {/* Cursor highlight spotlight */}
        {isHovered && (
          <div
            className="pointer-events-none absolute inset-0 z-20 transition-opacity duration-300"
            style={{
              background: `radial-gradient(350px circle at ${mousePos.x}px ${mousePos.y}px, rgba(216, 93, 58, 0.12), transparent 70%)`,
            }}
          />
        )}
